...
# 設定
TARGET_COUNT = 100
DB_PATH = Path('/Users/claw/.openclaw/workspace/car28_scraper_v4.db')
EXCEL_PATH = Path('/Users/claw/.openclaw/workspace/28car_潛客_完整版_v4.xlsx')
...
def extract_seller(lines, phone_idx):
    """提取賣家名稱（通常在電話附近）"""
    # 搜索前後50行
    search_range = range(max(0, phone_idx-50), min(len(lines), phone_idx+50))
    search_text = ' '.join(lines[i] for i in search_range)
    
    # 常見賣家名稱模式
    # 模式1: XXX 電話:12345678
    seller_patterns = [
        r'([\u4e00-\u9fa5]{1,4}生?)\s*電話\s*:?\s*\d{8}',  # 陳生 電話:12345678
        r'([\u4e00-\u9fa5]{1,4}小姐?)\s*電話\s*:?\s*\d{8}',  # 李小姐 電話:12345678
        r'([\u4e00-\u9fa5]{1,4}太太?)\s*電話\s*:?\s*\d{8}',  # 王太太 電話:12345678
        r'([A-Z][a-z]+\s?[A-Z][a-z]*)\s*電話\s*:?\s*\d{8}',  # BEN NG 電話:12345678
        r'([A-Z]{2,})\s*電話\s*:?\s*\d{8}',  # TOMY 電話:12345678
        r'([\u4e00-\u9fa5]{1,4})\s*\d{8}',  # 張 12345678
        r'([A-Z][a-z]*)\s*,?\s*電話\s*:?\s*\d{8}',  # Jack, 電話:12345678
    ]
    ...
    for pattern in seller_patterns:
        ...
        if match:
            ...
            # 清理常見後綴
            seller = re.sub(r'(電話|查詢|聯絡|WhatsApp|whatsapp|WECHAT|wechat).*$', '', seller, flags=re.IGNORECASE).strip()
            if seller and len(seller) <= 20:
                return seller
    ...
    for j in range(max(0, phone_idx-10), min(len(lines), phone_idx+10)):
        line = lines[j].strip()
        # 匹配簡短中文名（1-4字）或英文名
        if re.match(r'^[\u4e00-\u9fa5]{1,4}生?$', line) or re.match(r'^[\u4e00-\u9fa5]{1,4}小姐?$', line):
            return line
        if re.match(r'^[A-Z][a-z]+\s?[A-Z][a-z]*$', line) and len(line) <= 20:
            return line
        if re.match(r'^[A-Z]{2,}$', line) and len(line) <= 10:
            return line
...
def save_lead(cursor, conn, brand, phone, email, model, seller, description, source, page):
    """保存潛客資料"""
    try:
        cursor.execute('''
            INSERT OR IGNORE INTO car28_leads (brand, phone, email, model, seller, description, source, page, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (brand, phone, email, model, seller, description, source, page, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
        conn.commit()
...
def export_to_excel(cursor):
    """導出到Excel"""
    cursor.execute('''
        SELECT brand as 車廠, phone as 電話, seller as 賣家, email as 電郵, 
               model as 車型, description as 描述, source as 來源, page as 頁碼, created_at as 創建時間
        FROM car28_leads 
    ...
    cleaned_rows = []
    for row in rows:
        cleaned = tuple(clean_text(str(cell)) if cell else '' for cell in row)
        cleaned_rows.append(cleaned)
    
    df = pd.DataFrame(cleaned_rows, columns=['車廠', '電話', '賣家', '電郵', '車型', '描述', '來源', '頁碼', '創建時間'])
    df.to_excel(EXCEL_PATH, index=False, engine='openpyxl')
    return len(df)

def main():
    print("="*60)
    print("🚗 28car 爬蟲 - 完整版 v4")
    print("="*60)
    print("功能：寬鬆條件 + 自動提取車廠 + 自動提取賣家 + 目標100條\n")
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    ...
                    for phone in phones:
                        ...
                        seller = extract_seller(lines, i)
                        email = extract_email('\n'.join(lines[max(0,i-30):i+30]))
                        description = extract_description(lines, i)
                        source = f"page_{page_num}_line_{i}"
                        
                        if save_lead(cursor, conn, brand, phone, email, model, seller, description, source, page_num):
                            total_new += 1
                            page_new += 1
                            brand_str = brand if brand else "(未識別)"
                            seller_str = seller if seller else "(無)"
                            model_str = model[:20] if model else "(無)"
                            print(f"    ✅ {phone} | {brand_str} | {seller_str}")
    ...
    cursor.execute('SELECT COUNT(*) FROM car28_leads WHERE seller != "" AND seller IS NOT NULL')
    with_seller = cursor.fetchone()[0]
    ...
    print(f"   有賣家：{with_seller} 條 ({100*with_seller//max(1,final_count)}%)")
    print(f"   有電郵：{with_email} 條 ({100*with_email//max(1,final_count)}%)")
    
    print(f"\n🏭 車廠分布（前10）：")
    cursor.execute('SELECT brand, COUNT(*) FROM car28_leads WHERE brand IS NOT NULL GROUP BY brand ORDER BY COUNT(*) DESC LIMIT 10')
