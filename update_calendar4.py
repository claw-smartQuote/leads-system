...
set newEvent to make new event with properties {{summary:"🚗 保單到期提醒: {plate}", start date:startDate, end date:endDate, allday event:true, description:"被保人: {insured} | 車牌: {plate} | 保單到期日: {month}/{day} | 保險公司: 永誠"}}
set recurrence of newEvent to "FREQ=YEARLY;INTERVAL=1"
        ...
        if result.returncode == 0:
            ...
            failed.append((plate, result.stderr[:100]))
        ...
        failed.append((policy.get('plate_number', 'unknown'), str(e)[:50]))
